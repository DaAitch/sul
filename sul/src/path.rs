...
fn next_token_match_source(
    ...
    let none_arm_source = match path_item.route_end {
        Some(route_end) => {
            // TODO: other methods ...
            let request_type_name = format!("Get{}Request", super::ucc(route_end.path));
            ...
            let controller_fn_id = &route_end.controller_fn_id;
...
fn insert_into<'a>(
    ...
    outstanding_path: &'a str,
    route_end: &'a Route,
) {
    for token in outstanding_path.split(SEP).skip(1) {
        let (key, template_name) = as_item_type(token);
        let next_path_item = path_item
            .sub_items
            .entry(key)
            .or_insert_with(move || PathItem {
                parent_template_name: template_name,
                ..Default::default()
            });
...
pub struct Route<'a> {
    ...
    /// e.g: `self.get_users`
    ///
    /// will then append: `( parameters.. )`
    pub controller_fn_id: syn::Ident,
    pub response_type_id: syn::Ident,
}

struct PathItem<'a> {
    parent_template_name: Option<&'a str>,
    route_end: Option<&'a Route<'a>>,
    sub_items: HashMap<PathItemType<'a>, PathItem<'a>>,
}
...
fn as_item_type<'a>(path_item: &'a str) -> (PathItemType<'a>, Option<&'a str>) {
    if path_item.starts_with(TEMPLATE_START) && path_item.ends_with(TEMPLATE_END) {
        (
            PathItemType::Template,
            Some(&path_item[1..path_item.len() - 1]),
        )
    } else {
        (PathItemType::Path(path_item), None)
    }
}
